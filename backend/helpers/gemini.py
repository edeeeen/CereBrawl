...
#print(response.text)
'''
The call to the Gemini API to generate a full quiz that is within the format specificed in the prompt. The quiz is generated based on the topic provided as an argument.
Parameters:
- topic: str (the subject of the quiz, e.g., "biology", "history", "math")
Returns:
- response: the raw response from the Gemini API containing the full quiz text
'''
...
"""
    ...
    return response
'''
Uses the previous functions returned value of the full quiz to pick a single question based off of the full quiz and the difficulty
Parameters:
- fullQuiz: str (the full quiz text generated by Gemini)
